...
class SnapGridView extends BsGridView
{
	...
	public function init()
	{
		...
		parent::init();
		$this->snapBaseScriptUrl=Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('backend.widgets.assets')).'/snapgridview';
		...
		if($this->selectableColumns === true)
		{
			...
			foreach($this->columns as $pos=>$column)
			{
				if(is_subclass_of($column, 'CDataColumn'))
				{
					...
					$this->_selectableColumnsList[$column->name] = 
						$column->header ? 
	...
	protected function _setSelectedColumns()
	{
		if(isset($_GET[$this->dataProvider->modelClass]['SelectedColumns'])) {
			...
			Yii::app()->session['SnapGridView_'.$this->dataProvider->modelClass.'_SelectedColumns'] = 
